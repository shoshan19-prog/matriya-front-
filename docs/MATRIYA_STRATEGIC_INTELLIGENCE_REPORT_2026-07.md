...
# נספח א' — הערכת שווי עכשווית מעודכנת (עברית)

**תאריך:** 2026-07-05
**הנחה חדשה שנמסרה על-ידי החברה:** פרסקו (Fresco) תהיה הלקוח המשלם הראשון — תשלום של כ-100,000 (מטבע לא צוין — ₪ או $, ראו רגישות בהמשך) עבור שימוש במערכת, ותבצע את הוולידציה של הפלטפורמה.
**סטטוס ההנחה:** לא מאומת — אין בקוד או במסמכים חוזה, הזמנת רכש או תשלום בפועל. ההערכה להלן מותנית במימוש ההתחייבות.

## א.1 מה הנתון החדש משנה

עסקת פרסקו, אם תמומש, סוגרת בבת אחת שלושה מהפערים הקריטיים שזוהו בדוח:

1. **הוכחת נכונות-לשלם (Sales: 1/10 → 3-4/10)** — לראשונה יש מחיר שמישהו מוכן לשלם עבור המערכת.
2. **ולידציה על דאטה אמיתי** — פרסקו מחזיקה בדיוק את הקורפוס שהמערכת נבנתה עבורו (פורמולציות INTUMESCENT, דוחות שריפה, תיקיות דגימות 001-095+). ולידציה מוצלחת אצלה היא בדיוק "תרחיש B" מהדוח (פלטפורמה עובדת עם KUR>0), בתוספת תשלום.
3. **מקרה בוחן (Case Study)** — לקוח ראשון בתחום ציפויי הגנה מאש הוא כרטיס הכניסה לשיחות עם הלקוח השני והשלישי.

## א.2 הסתייגות מהותית — לקוח קשור (Related Party)

**מאומת מהקוד:** פרסקו אינה לקוח זר. הקורפוס של המערכת הוא הדאטה של פרסקו עצמה, דומיין המייל של המערכת הוא frescomailserver.xyz, וחשבון ה-HubSpot היחיד הוא "fresco paints". משקיע מקצועי יתייחס לעסקה כ**ולידציה טכנולוגית מצוינת אך ולידציית שוק חלקית בלבד**, ויחיל דיסקאונט של 30%-50% על הסיגנל המסחרי לעומת לקוח ראשון בלתי-תלוי. הערך האמיתי של העסקה הוא ה-Reference וההוכחה שהמנוע עובד על דאטה תעשייתי אמיתי — לא ההכנסה עצמה.

## א.3 שווי עכשווי מעודכן (מוערך)

| מצב | תנאים | שווי מוערך (Pre-Money) |
|---|---|---|
| **היום — הצהרת כוונות בלבד** | אין חוזה חתום, אין תשלום, הקורפוס עדיין לא נקלט (KUR=0) | **$0.8M – $1.8M** (עלייה קלה מ-$0.5M–$1.5M בדוח — כוונת תשלום מוצהרת שווה מעט, לא הרבה) |
| **חוזה חתום + מקדמה** | הסכם ולידציה חתום עם אבני דרך, תשלום ראשון התקבל | **$1.5M – $2.5M** (אם ₪100K ≈ $27K) / **$2M – $3.5M** (אם $100K) |
| **ולידציה הושלמה בהצלחה** | הקורפוס נקלט, KUR>0, שאילתות אמיתיות נענות עם מקורות, מלוא ה-100,000 שולם, דוח ולידציה מתועד | **$3M – $5M** — שילוב של תרחיש B (פלטפורמה עובדת) + תרחיש C חלקי (לקוח משלם, בדיסקאונט צד-קשור) |
| **ולידציה + לקוח חיצוני ראשון** | לקוח בלתי-תלוי שני חותם על בסיס מקרה הבוחן של פרסקו | **$5M – $12M** — תרחיש C המלא מהדוח, ללא דיסקאונט |

**הנחות חישוב:** מכפילי Seed מקובלים ל-SaaS ורטיקלי בשלב הכנסה ראשונה (פי 30-80 על ARR זעיר משקפים ערך אופציה, לא מתמטיקת הכנסות); דיסקאונט צד-קשור 30%-50% על הסיגנל המסחרי; ההנחה שהוולידציה כוללת את קליטת הקורפוס, אכיפת שער המחקר מקצה-לקצה וסגירת חובות האבטחה (סעיפים 1-4 ב"קריטי" במפת הדרכים) — בלעדיהן הוולידציה תיכשל טכנית ("אין במערכת מידע תומך") והשווי חוזר לשורה הראשונה בטבלה.

## א.4 שורה תחתונה

עסקת פרסקו היא בדיוק המהלך הנכון — היא הופכת את "תרחיש B" מהדוח למסלול ממומן במקום משימה פנימית. אבל היא **מזרז, לא קיצור דרך**: השווי לא קופץ ביום החתימה אלא ביום שבו הוולידציה עוברת בפועל על הקורפוס האמיתי. המלצה: לעגן את ה-100,000 בהסכם ולידציה כתוב עם אבני דרך מדידות (קליטת קורפוס → KUR>0 → 3 החלטות אמיתיות שנתמכו במערכת, לפי כלל ההחלטה שהקוקפיט עצמו מגדיר), ולתעד הכול — ההסכם הזה הוא המסמך החשוב ביותר בחדר הנתונים (Data Room) של הסבב הבא.

*כל הסכומים בסעיף זה הם הערכות אנליסט (ESTIMATED); קיום העסקה עצמה — לא מאומת (UNKNOWN) נכון למועד הכתיבה.*
