...
FIRST_MEASUREMENT         5   +0.452   ₪4,500     0.100     0.40
OPEN_ASSET                9   +0.194   ₪2,000     0.097     0.11
MORE_MEASURED             4   +0.118   ₪3,000     0.039     0.50
NEW_PRODUCT_QUALITATIVE   7   +0.101   ₪  800     0.127     0.00
```
*(FIRST_MEASUREMENT is n=5 after iteration 1 below — a **candidate** primitive, not yet a law.)*
...
## Acquisition iteration 1 — the loop in action (recommend → approve → scout → recompute)

The governed loop ran one real iteration. PROTEUS recommended the top-ROI events; the human approved a ROI-ordered scout (`SET_TIME → SIEVE → PULL_OFF`); the read-only scout returned:

| Event | Verdict | Action |
|-------|---------|--------|
| FIRST_SET_TIME (Set/Cure) | **ABSENT** | must **GENERATE** (run a Vicat/set-time test) |
| FIRST_SIEVE_CURVE (Granulometry) | **PRESENT** (real sieve curves + Malvern laser PSD) | **RETRIEVE** ✓ done |
| FIRST_PULL_OFF (Adhesion) | **ABSENT** (only a never-executed Applus quote) | must **GENERATE** (run a pull-off test) |

So one event was acquired by **retrieval** and folded into the corpus — a real transformation:
```
step 8  raw-material QC  Granulometry / Fractions  0.25 → 0.65  (+0.40, first-measured, surprise 0.7)
```
Granulometry went from 0 measured → **2 measured, confidence 0.65**. The FIRST_MEASUREMENT primitive refined on real data: **n=5, avg ΔK 0.452** (still a *candidate* primitive, not a law — exactly the discipline: `n=5` says "candidate", not "Scientific Law").

The deeper finding: **two of the three highest-VoI events (set-time, adhesion) do not exist as data and must be generated by new lab tests.** PROTEUS's recommendation flips from *retrieve* to **GENERATE** — it now recommends *running an experiment*, the highest-knowledge-per-shekel one (`FIRST_SET_TIME`, ₪600, ROI 0.75), pending approval. This echoes the project's original DOE finding: the corpus is rich in *inputs* but the *response* is structurally missing, so the most valuable knowledge must be **created**, not retrieved.
